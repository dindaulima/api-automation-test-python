class TestCards:
    #TC1 - Create a card on a valid list → expect 200, correct fields (name, idList, due: null).
    def test_create_card_on_a_valid_list(self, api_client, test_list, test_card):
        assert test_card["name"].startswith("pytest-card-")
        assert test_card["idList"] == test_list["id"]
        assert test_card["due"] is None

    #TC2 - Create a card with a due date → verify due field returned correctly.
    def test_create_card_with_due_date(self, api_client, test_list):
    ...
    #TC3 - Get a card by ID → matches created data.
    def test_get_card_by_id(self, api_client, test_card):
    ...
    #TC4 - Update card name/description → verify persisted.
    def test_update_card_name_and_description(self, api_client, test_card):
    ...
    #TC5 - Move a card to a different list → verify idList updated.
    def test_move_card_to_different_list(self, api_client, test_board, test_card):
    ...
    #TC6 - Archive a card → verify closed: true.
    def test_archive_card_sets_closed_true(self, api_client, test_card):
        update_response = api_client.update_card(test_card["id"], closed="true")

        assert update_response.status_code == 200
        assert update_response.json()["closed"] is True

    #TC7 - Delete a card → verify GET returns 404.
    def test_delete_card_removes_it(self, api_client, test_card):
    ...
    #TC8 - Create a card on a non-existent list → expect 400/404.
    def test_create_card_on_nonexistent_list_is_rejected(self, api_client):
    ...
    def test_add_label_to_card(self, api_client, test_board, test_card):
    ...
    def test_add_member_to_card(self, api_client, test_card):
    ...
    def test_add_comment_to_card(self, api_client, test_card):
    ...
    def test_create_card_with_invalid_due_date_is_rejected(self, api_client, test_list):
