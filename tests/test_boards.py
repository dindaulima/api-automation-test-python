...
class TestBoards:
    ...
    def test_create_board_with_valid_name(self, api_client, test_board):
        assert test_board["name"].startswith("pytest-board-")
        assert test_board["closed"] is False
        assert "id" in test_board

    #TC2 - Get board by ID
    def test_get_board_by_id(self, api_client, test_board):
    ...
    def test_update_board_name(self, api_client, test_board):
    ...
    def test_create_board_with_empty_name_is_rejected(self, api_client):
        response = api_client.create_board(name="")

        assert response.status_code == 400

    # TC5 - Get board with non-existent ID → 404
    def test_get_nonexistent_board_returns_404(self, api_client):
        response = api_client.get_board("000000000000000000000000")

        assert response.status_code == 404

    # TC6 - Delete board → verify it's gone
    def test_delete_board_removes_it(self, api_client, test_board):
    ...
    def test_get_board_with_invalid_token_returns_401(self, test_board):
